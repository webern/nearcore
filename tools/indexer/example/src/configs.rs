...
pub(crate) fn init_logging() {
    let env_filter = EnvFilter::new(
        "nearcore=info,indexer-example=info,tokio_reactor=info,near=info,near=error,stats=info,telemetry=info,indexer_example=info,indexer=info,near-performance-metrics=info",
    );
